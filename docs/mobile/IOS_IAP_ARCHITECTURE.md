...
Ownership-conflict response note: `apple-purchase-verify` must not treat a durable reconciliation-required row as a client success merely because the processor returned `processed` or `already_processed` evidence. StoreKit finish permission, `ok: true`, and native-review entitlement guidance require an accepted processor row with no reconciliation requirement, no retryable failure, no ownership, account-token, payload, product, subscription-state, or environment conflict, and for idempotent already-processed rows a same-account ownership or entitlement-refresh signal. Ownership conflicts return HTTP `409` so the iOS bridge maps them to `accountConflict` and leaves the transaction unfinished.

Production-sandbox reconciliation note: TestFlight and App Review sandbox Apple purchases verified against the production backend must project Pro only through `billing.apple_native_sandbox_entitlements`; `public.entitlements` remains the live website entitlement and should stay Free for those sandbox purchases. Apple reconciliation checks therefore require an explicit deployment mode and must use the native sandbox projection for production/sandbox state, while staging/sandbox and production/production Apple state continue to use `public.entitlements`. After a same-account purchase verification binds an Apple original transaction chain, the backend may resolve an earlier verified `notification:%` provider event parked as `unbound_original_transaction` for the same original-transaction fingerprint. This repair is not a replay, ownership transfer, or manual entitlement edit.
| `updated_at` | timestamptz | Service-role update timestamp. |
