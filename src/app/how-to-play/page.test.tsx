...
describe("HowToPlayPage", () => {
  it("uses player-facing scoring and account copy", () => {
    ...
    expect(screen.getByText(/tap countries if you want help/i)).toBeVisible();
    expect(screen.getByText(/Choose the answer that fits the map/i)).toBeVisible();
    ...
    const patternAtlasCard = screen
      .getAllByRole("heading", { name: "Pattern Atlas" })
      .map((heading) => heading.closest("article"))
      .find((card): card is HTMLElement => Boolean(card && within(card as HTMLElement).queryByRole("link", { name: /Play Pattern Atlas/i })));
    const orderAtlasCard = screen
      .getAllByRole("heading", { name: "Order Atlas" })
      .map((heading) => heading.closest("article"))
      .find((card): card is HTMLElement => Boolean(card && within(card as HTMLElement).queryByRole("link", { name: /Play Order Atlas/i })));
    expect(patternAtlasCard).toBeTruthy();
    expect(orderAtlasCard).toBeTruthy();
    expect(within(patternAtlasCard!).getByRole("link", { name: /Play Pattern Atlas/i })).toHaveAttribute("href", "/play/pattern-atlas");
    expect(within(orderAtlasCard!).getByRole("link", { name: /Play Order Atlas/i })).toHaveAttribute("href", "/play/order-atlas");
    ...
    expect(screen.getByRole("link", { name: /Play Mystery Map sample/i })).toHaveAttribute("href", "/play/mystery-map");
    expect(screen.queryByRole("link", { name: /Try Mystery Map sample/i })).not.toBeInTheDocument();
  ...
  it("keeps button-style links out of prose underline rules", () => {
    expect(styles).toContain(".how-page a:not(.button):not(.button-secondary):not(.icon-button):not(.game-library-card-link)");
    expect(styles).not.toContain(".how-page a:not(.button):not(.button-secondary):not(.icon-button),");
