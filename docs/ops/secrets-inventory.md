...
| `SUPABASE_STAGING_DB_URL` | Yes | Staging SQL validation runner | Prefer prompt mode for human validation: `pnpm ops:supabase:staging-rls -- --prompt`. Export manually only for automation. Prefer the direct connection string over transaction-pooler URLs. Do not print, paste, log, or commit. |
