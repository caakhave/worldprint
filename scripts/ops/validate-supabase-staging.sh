...
if [[ -n "${SUPABASE_PROD_DB_URL:-}" || -n "${SUPABASE_PRODUCTION_DB_URL:-}" || -n "${PRODUCTION_DATABASE_URL:-}" ]]; then
  echo "Refusing to run while a production-looking database URL variable is present." >&2
  echo "Unset production database variables before running staging validation." >&2
  exit 1
fi
...
if [[ -z "${SUPABASE_STAGING_DB_URL:-}" ]]; then
  ...
  if [[ -n "${DATABASE_URL:-}" || -n "${SUPABASE_DB_URL:-}" ]]; then
    echo "Refusing to use generic database URL variables for staging validation." >&2
  fi
