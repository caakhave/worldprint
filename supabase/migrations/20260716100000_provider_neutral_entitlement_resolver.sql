  ...
  select
    ...
    case
      when cr.status = 'active'
        ...
        and (cr.current_period_start is null or (select as_of from requested) >= cr.current_period_start)
    ...
    case
      ...
      when cr.status = 'active'
        and (
          ...
          or (cr.current_period_start is not null and (select as_of from requested) < cr.current_period_start)
