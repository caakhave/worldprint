...
def test_mystery_map_signed_out_sample_selection_smoke(desktop_page, target_base_url: str):
    ...
    expect(desktop_page.get_by_test_id("indicator-answer-panel")).to_be_visible()
    answers = desktop_page.get_by_role("group", name="What does this map measure?").get_by_role("button")
