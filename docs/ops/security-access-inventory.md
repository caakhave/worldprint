...
- `scripts/ops/validate-supabase-staging.sh` provides a safe staging-only SQL validation path. It requires `SUPABASE_STAGING_DB_URL`, uses `supabase db query --db-url`, splits multi-statement SQL into discrete CLI calls, and does not use linked project state.
