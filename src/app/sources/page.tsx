...
export default function SourcesPage() {
  const reviewSummary = editorialReview as {
  ...
  const formatStatusCounts = (counts: Record<string, number>) =>
    Object.entries(counts)
      ...
      .join(", ");
  const dailyReadyCount = reviewSummary.approvedStatusCounts?.daily_eligible ?? 0;
  ...
  return (
    <section className="text-page page-shell">
      ...
      <p className="lead">
        Can You Geo? generates static content artifacts at build time for Mystery Map. Gameplay uses one explicit reference year per
        indicator and excludes World Bank aggregate pseudo-countries.
      </p>
      <h2>Data Sources</h2>
      <p>
      ...
      <p>
        Version {manifest.contentVersion}. The candidate bank contains {reviewSummary.candidateCount ?? manifest.indicators.length}
        indicators; {reviewSummary.approvedCount ?? manifest.indicators.length} passed the current source-data gate and{" "}
        {reviewSummary.draftCount ?? 0} are draft-held for coverage or source issues. The playable round bank contains{" "}
        {manifest.rounds.length} generated rounds after Needs-review and Retired maps are excluded. {dailyReadyCount} maps are
        Daily-ready. Daily manifests are pre-generated for the current static archive window so old archive pages do not reshuffle
        when the player opens them later.
      </p>
      ...
      <p>
        ...
        country registry. Mystery Map never mixes each country&apos;s individually latest value into one unlabeled map.
      </p>
      ...
      <p>
        ...
        visual-similarity warnings across source-valid indicators. Close distractors are useful on higher tiers, but very high
      ...
      <p>
        A World Bank indicator can be source-valid without being good Daily material. Mystery Map now gives each indicator an
      ...
      <p>
        The build also writes candidate scorecards for coverage, freshness, unit clarity, map interest, and ambiguity/correlation,
        plus a content status diff between builds. Those reports help editors scale the catalog without auto-approving weak maps;
        curated editorial status remains the source of truth.
      </p>
      <p>
        Current candidate editorial status counts: {formatStatusCounts(reviewSummary.statusCounts)}. Source-valid status counts:{" "}
        {formatStatusCounts(reviewSummary.approvedStatusCounts ?? reviewSummary.statusCounts)}.
